...
from formula_visualization import dnf2conj_list, get_used_preds, check_for_additional_nodes
...
from RL2DT.hyperparams import BRANCHING, NUM_PROGRAMS, NUM_ACTIONS, ENV_TYPE, ORDER, \
                        BASE_NUM_DEMOS, NUM_CLUSTERS, SPLIT, MAX_DIVERGENCE, \
                        CLUSTER_DEPTH, MAX_DEPTH, NUM_ROLLOUTS, TOLERANCE
import sys
from RL2DT.MCRL import modified_mouselab_new
sys.modules['modified_mouselab_new'] =modified_mouselab_new

BLANK = "\n                    " #used for printing purposes
ITERS = 1 #how many times to run the algorithm to establish consistency
...
def wrapper_get_elbows(algorithm, **kwargs):
    ...
    if algorithm == 'binary':
        ...
        dsl_path = os.getcwd() + '/demos/' + kwargs['elbow_dsl_file'] + '.pkl'
...
def save_adaptive(obj, divergence, depth, val, val_ratio, 
    ...
    DIVERGENCE = '_divergence_'+str(divergence)
    DEPTH = '_depth_'+str(depth)
    ...
    ENV_TYPE = '_' + env
    str_n = '_' + str(num) if num is not None else ''
    filename = NDEMO + ENV_TYPE + DIVERGENCE + DEPTH + VALID + NCLUST + str_n
...
def formula_value_speedup(formula, expert_reward, max_divergence, tolerance,
                          num_rollouts, normalizer, stats, num):
    """
    ...
    if formula.program in formula_values.keys():
        ...
        if formula_divergence - tolerance <= max_divergence and run != num:
            diff_run = True
...
def is_best_formula(formula_divergence, formula_nodes, prev_formula_divergence, 
                    prev_formula_nodes, max_divergence, tolerance):
    '''
    ...
    low_div = formula_divergence < prev_formula_divergence - tolerance
    sim_div = abs(formula_divergence - prev_formula_divergence) <= tolerance
    suff_div = formula_divergence - tolerance <= max_divergence
    less_cmpx = formula_nodes < prev_formula_nodes
            
    if suff_div and (low_div or (sim_div and less_cmpx)):                   
        return True
    return False


def interpret_adaptive(num_demos, max_divergence, max_depth, tolerance,
                       num_clusters, demos, expert_reward, num_rollouts, 
                       normalizer=None, validation_demos=None, num_samples=10, 
                       clustering='DSL', use_DSL_data=True, dsl_path=None, 
                       env_name='', num=None, info='', save=True, **kwargs):
    """
    Try interpreting the behavior of (possibly an RL) policy in the spirit of 
    ...
    init_points = len(demos['pos'])
    no_formula = True 
    valid = 'cluster' if not(validation_demos) else 'file'
    if normalizer == None:
        normalizer = expert_reward
...
def get_formula_nodes(formula):
    ...
    preds_in_dnfs = dnf2conj_list(program)
...
def choose_interpretable(data_by_clusters):
    ...
    if best:
        ...
        stats = {}
        ks = ['points', 'all_points', 'clusters', 'depth', 'nodes', 'return', 'expert_return']
        for key in ks:
    ...
    for i in range(ITERS):
        ...
        for n_cl in candidate_numbers:
            ...
            data_by_clusters[n_cl] = {'formula': formula, 'points': stats[0],
                                      'all_points': stats[1], 'clusters': stats[1], 
                                      'depth': stats[2], 'nodes': stats[3], 
                                      'return': stats[4], 'expert_return': stats[5]}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--interpret_size', '-i',
                        ...
                        default=MAX_DEPTH)
    parser.add_argument('--max_divergence', '-l',
                        type=float,
    ...
    data_by_clusters = wrapper_interpret(algorithm=args.algorithm, 
                                         ...
                                         max_divergence=args.aspiration_level,
