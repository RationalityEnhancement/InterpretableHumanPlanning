import numpy as np
from RL2DT.MCRL.modified_mouselab_new import approx_max, approx_min
...
def has_leaf_highest_level_value(state, action):
    ...
    max_leaf_val = approx_max(state.reward_function(leaf_depth))
...
def has_root_highest_level_value(state, action):
    ...
    max_root_val = approx_max(state.reward_function(1))
...
def has_leaf_smallest_level_value(state, action):
    ...
    leaf_depth = obs_leaves_node[0].depth
    min_leaf_val = approx_min(state.reward_function(leaf_depth))
...
def has_root_smallest_level_value(state, action):
    ...
    min_root_val = approx_min(state.reward_function(1))
...
def has_child_highest_level_value(state, action):
    ...
    max_val = approx_max(state.reward_function(node.depth+1))
...
def has_child_smallest_level_value(state, action):
    ...
    min_val = approx_min(state.reward_function(node.depth+1))
...
def is_previous_observed_max(state, action):
    ...
    """
    return bool(state.is_previous_max())
...
def is_previous_observed_max_level(state, action, dep):
    ...
    if prev_obs_node == None:
        return False
    return prev_obs_node.value == approx_max(state.reward_function(dep))
...
def diff_best_2ndbest(state, action, num):
    path_expected_values = sorted(state.get_path_expected_values())
    difference = path_expected_values[-1] - path_expected_values[-2]
