...
def generate_DSL_file(pos_input, neg_input, info):
    ...
    dsl_data = os.getcwd() + '/demos/DSL_' + info + '.pkl'
...
def get_clustering_values(machine, cluster_nums, info):
    ...
    for num_cl in cluster_nums:
        ...
        clustering = machine.interpret_clusters(verbose=False)
        score = machine.evaluate_clusters(clustering, verbose=False)
...
def save_clustering_values_to_file(data, cluster_sizes, info=''):
    ...
    '''
    cwd = os.getcwd()
    folder_path = cwd+'/interprets_formula/elbow/'
    import csv
...
def get_elbows(demo_path, num_demos, num_candidates, environment='standard',
    ...
    if dsl_data == None:
        ...
        path_dsl_data = os.getcwd() + '/demos/' + dsl_data + '.pkl'
