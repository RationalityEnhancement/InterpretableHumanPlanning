...
from formula_visualization import dnf2conj_list as d2l, prettify
...
from evaluate_procedure import compute_score_people
import os
import re
import argparse
import pickle
...
def best_until(pred, until_conds, raw_phi, p_envs, p_actions, p_pipeline, unless=False):
    ...
    for ind, u in enumerate(until_conds):
        ...
        res = compute_score_people(tuple(tuple(e) for e in p_envs), 
                                   pipeline=(tuple(p_pipeline[0][0]), p_pipeline[0][1]), 
                                   people_acts=tuple(tuple(a) for a in p_actions), 
                                   formula=tst_raw,
                                   verbose=False)
        if res[0] > best_lik:
            best_u = u
...
def sizeBig(formula):
    ...
    if loop != '':
        l_u = loop.split(' UNLESS ')
        u = '' if len(l_u) < 2 else l_u[1]
        l = l_u[0]
        size_f += size(l) + size(u)
    return size_f
...
def DNF2LTL(phi, trajs, predicate_matrix, allowed_predicates, p_envs, p_actions,
    ...
    for num, eqc in enumerate(equiv_classes):
        ...
        compute_score_people.cache_clear()
